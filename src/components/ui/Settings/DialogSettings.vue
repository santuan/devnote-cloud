...
<template>
  <DialogRoot v-model:open="show_settings">
    ...
    <DialogPortal>
      ...
      <DialogContent
        class="font-mono fixed inset-0 h-screen w-full mx-auto bg-background rounded py-1 md:p-4 focus:outline-none z-[100]"
      >
        <div class="relative md:mt-6 max-w-4xl mx-auto">
          ...
          <DialogDescription class="mt-1 text-sm px-3  text-muted-foreground">
            ...
            <span v-memo="[isMobile]">{{ isMobile ? "touch" : t("settings.mouseKeyboard") }}.</span>
          </DialogDescription>
          <DialogClose
            class="absolute inline-flex items-center justify-center appearance-none top-0 text-foreground hover:bg-secondary/80 right-0 size-7 focus:shadow focus:outline-none focus-visible:ring-1 focus-visible:ring-primary focus-visible:text-primary"
            aria-label="Close"
          >
            <X />
          </DialogClose>
        ...
        <div
          class="w-full border h-auto mt-2 md:mt-6 max-w-4xl mx-auto overflow-y-auto border-secondary focus-within:ring-primary/70 focus-within:ring-1 max-h-[80vh] sm:max-h-[75vh] focus:!ring-primary focus:!ring-2 outline-none"
          style="--scrollbar-size: 10px"
        >
          <article class="max-w-full py-3 pl-3 mx-auto prose dark:prose-invert">
