...
<template>
  <DialogRoot v-model:open="modal.show_video_url_dialog">
    <DialogPortal>
      ...
      <DialogContent
        class="font-mono fixed top-6 translate-y-0 sm:top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[500px] translate-x-[-50%] sm:translate-y-[-50%] rounded-[6px] bg-background p-6 shadow focus:outline-none z-[100] border border-secondary"
      >
        ...
        <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
          <input
            ...
            required
            placeholder="https://"
            class="flex h-10 w-full rounded-md border border-secondary bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
          >
